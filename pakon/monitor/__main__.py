...
def get_dev_mac(ip):
    ...
    if not s:
        ...
        return ("", "")
    res = re.search(
        r"dev\s+([^\s]+)\s+.*lladdr\s+((?:[a-f\d]{1,2}\:){5}[a-f\d]{1,2})", s
    )
    if not res:
        logging.warn(
            "no match for dev&mac in output of `ip neigh show {}`: {}".format(ip, s)
        )
        return ("", "")
    dev = res.groups()[0]
    mac = res.groups()[1]
...
def new_device_notify(mac, iface):
    ...
    thread = threading.Thread(
        target=new_device_notify_thread,
        args=(
            mac,
            iface,
        ),
...
def handle_flow(data, con):
    ...
    if (
        data["app_proto"] == "dns"
        or int(data["flow"]["bytes_toserver"]) == 0
        or int(data["flow"]["bytes_toclient"]) == 0
    ):
...
def handle_tls(data, con):
    ...
    if not hostname:
        return
    con.execute(
        'UPDATE traffic SET app_hostname = ?, app_proto = "tls" WHERE flow_id = ?',
...
def handle_http(data, con):
    if "hostname" not in data["http"].keys():
        return
    con.execute(
        'UPDATE traffic SET app_hostname = ?, app_proto = "http" WHERE flow_id = ?',
    ...
    def __init__(self):
        try:
            ...
            self.conntrack = subprocess.Popen(
                [
                    "/usr/bin/python3",
                    "/usr/libexec/suricata_conntrack_flows.py",
                    "/var/run/pakon.sock",
                ],
                shell=False,
...
def main():
    ...
    hard_limit = uci_get("pakon.monitor.database_limit", dtype=int, default=3000000)
    ...
    while True:
        try:
            ...
            if run_check:
                ...
                if count > hard_limit:
                    logging.warning(
                        "over {} records in the live database ({}) -> deleting",
                        hard_limit,
                        count,
                    )
                    con.execute(
