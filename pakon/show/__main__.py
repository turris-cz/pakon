...
_EPILOG = """Valid datetime specifier formats (for -s/-e options) are:
ABSOLUTE
...
def print_table(table):
    ...
    for line in table:
        print(
            "|"
            + " | ".join(
                "{:{}}".format(str(x), col_width[i]) for i, x in enumerate(line)
            )
            + "|"
        )
...
def arg_parser():
    ...
    parser.add_argument(
        "-e", "--end", help="End of time window", metavar="DT", type=timespec_valid
    )
    parser.add_argument(
        "-j",
        "--json",
    ...
    parser.add_argument(
        "-m",
        "--mac",
        help="Show just records for specified MAC address OR name "
        "(multiple such options can be specified)",
        action="append",
        metavar="MAC",
        type=mac_name_valid,
    )
    parser.add_argument(
        "-H",
        "--hostname",
        help="Show just records for specified (destination) hostname "
        "(multiple such options can be specified)",
        metavar="N",
        action="append",
    )
    parser.add_argument(
        "--no-filter",
        action="store_false",
        help="Don't apply filter to output (hides tracking, "
        "advertisements and other rubbish)",
    )
    parser.add_argument(
        "-A",
        "--aggregate",
...
def main():
    ...
    for i, arg in enumerate(sys.argv):
        if (arg[0] == "-") and len(arg) > 1 and arg[1].isdigit():
            sys.argv[i] = " " + arg
    args = arg_parser()
    query = vars(args)
