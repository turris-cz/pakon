...
def squash(from_details, to_details, rules):
    ...
    if from_details == "live":
        for row_mac in c.execute(
            "SELECT DISTINCT src_mac FROM live.traffic WHERE start < ? AND flow_id IS NULL",
            (start,),
        ):
            ...
            for row_hostname in c2.execute(
                ...
                (i, d) = squash_for_mac_and_hostname(
                    src_mac, hostname, from_details, to_details, start, rules
                )
                inserted += i
                deleted += d
        con.execute(
            "DELETE FROM live.traffic WHERE start < ? AND flow_id IS NULL", (start,)
        )
    else:
        for row_mac in c.execute(
            "SELECT DISTINCT src_mac FROM traffic WHERE details = ? AND start < ?",
            (
                from_details,
                start,
            ),
            ...
            for row_hostname in c2.execute(
                ...
                deleted += d
    logging.info(
        "deleted %d flows from detail level %s, inserted %d flows to detail level %d",
        deleted,
        str(from_details),
        inserted,
        to_details,
...
def squash_for_mac_and_hostname(
    src_mac, hostname, from_details, to_details, start, rules
):
    ...
    for row in result:
        ...
        if not current_flow:
            ...
            current_flow["dest_port"] == row["dest_port"]
            and current_flow["end"] + int(rules["window"]) > row["start"]
        ):
...
def load_archive_rules():
    ...
    if not rules:  # if there is no rule (old configuration?) - add one default rule
        ...
        logging.info(
            "no rules in configuration - using default {}".format(str(rules[0]))
        )
...
def archive():
    ...
    if count > hard_limit:
        logging.warning(
            "over {} records in the archive database ({}) -> deleting",
            hard_limit,
            count,
        )
        con.execute(
    ...
    for i in range(len(rules) + 1):
        ...
        logging.info(
            "{} flows remaining in archive on details level {}".format(
                c.fetchone()[0], i
            )
        )
