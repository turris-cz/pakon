...
def squash_live_for_mac_and_hostname(
    ...
    for row in c.execute(
        "SELECT start, duration, (start+duration) AS end, src_mac, COALESCE(app_hostname,dest_ip) AS hostname, (dest_port || '/' || lower(proto)) AS dest_port, app_proto, bytes_send, bytes_received FROM traffic WHERE src_mac = ? AND COALESCE(app_hostname,dest_ip) = ? AND flow_id IS NULL AND "
        + where_clause
        + " ORDER BY start",
...
def get_data_from_archive(con, where_clause, where_parameters, filter):
    ...
    c = con.cursor()
    for row in c.execute(
        "SELECT start, duration, src_mac, COALESCE(app_hostname, dest_ip) AS hostname, (dest_port || '/' || lower(proto)) AS dest_port, app_proto, bytes_send, bytes_received FROM archive.traffic WHERE "
        + where_clause,
        where_parameters,
    ):
...
def get_aggregate_data_from_archive(con, where_clause, where_parameters, filter):
    ...
    c = con.cursor()
    for row in c.execute(
        "SELECT start, SUM(duration) AS duration, src_mac, COALESCE(app_hostname, dest_ip) AS hostname, (dest_port || '/' || lower(proto)) AS dest_port, app_proto, SUM(bytes_send) AS bytes_send, SUM(bytes_received) AS bytes_received FROM archive.traffic WHERE "
        + where_clause
        + " GROUP BY src_mac, hostname, dest_port",
        where_parameters,
    ):
        if filter and is_ignored(row["hostname"]):
            continue
...
def aggregate_flows(flows):
    ...
    for i in range(1, len(flows)):
        if (
            current_flow[3] == flows[i][3]
            and current_flow[4] == flows[i][4]
            and current_flow[5] == flows[i][5]
        ):
...
def query(query):
    ...
    try:
        ...
        con.close()
        return "[]"
    (time_from, time_to, where_clause, where_parameters) = build_filter(
        query, {v: k for k, v in mac2name.items()}
    )
    aggregate = query["aggregate"] if "aggregate" in query else False
    filter = query["filter"] if "filter" in query else True
    if aggregate:
        flows = get_aggregate_data_from_archive(
            con, where_clause, where_parameters, filter
        )
        ...
        flows += sorted(
            get_data_from_live(con, where_clause, where_parameters, filter),
            key=lambda x: x[0],
        )
    for flow in flows:
...
def main():
    ...
    server = socketserver.UnixStreamServer(
        "/var/run/pakon-query.sock", ThreadedTCPRequestHandler
    )
